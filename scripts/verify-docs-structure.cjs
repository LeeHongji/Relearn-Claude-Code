...
function verifyDocsStructure(rootDir = process.cwd()) {
  ...
  if (!Array.isArray(inventory.topLevel) || inventory.topLevel.length < REQUIRED_SOURCE_INVENTORY_PATHS.length) {
    throw new Error('docs/public/source-inventory.json must contain a topLevel inventory array.');
  }
  ...
  for (const expectedPath of REQUIRED_SOURCE_INVENTORY_PATHS) {
    if (!inventoryPaths.has(expectedPath)) {
      throw new Error(`source inventory is missing expected top-level path: ${expectedPath}`);
    }
  }
  ...
  if (!siteIa.includes('English is the canonical source path.')) {
    throw new Error('site-information-architecture.md must document the English-first language strategy.');
  }
  ...
  for (const repoName of ['claude-code', 'how-claude-code-works', 'claude-code-from-scratch']) {
